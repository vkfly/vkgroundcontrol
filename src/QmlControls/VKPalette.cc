...
QList<VKPalette*>   VKPalette::_paletteObjects;

VKPalette::Theme VKPalette::_theme = VKPalette::Dark;

QMap<int, QMap<int, QMap<QString, QColor >>> VKPalette::_colorInfoMap;

QStringList VKPalette::_colors;

VKPalette::VKPalette(QObject* parent) :
    QObject(parent),
    _colorGroupEnabled(true) {
    if (_colorInfoMap.isEmpty()) {
        _buildMap();
    }
    // We have to keep track of all VKPalette objects in the system so we can signal theme change to all of them
...
void VKPalette::_buildMap() {
    ...
    DECLARE_QGC_SINGLE_COLOR(titleColor, "#3bae36")
...
void VKPalette::setColorGroupEnabled(bool enabled) {
    _colorGroupEnabled = enabled;
    emit paletteChanged();
}

void VKPalette::setGlobalTheme(Theme newTheme) {
...
void VKPalette::_signalPaletteChangeToAll() {
    // Notify all objects of the new theme
    for (VKPalette *palette : std::as_const(_paletteObjects)) {
        palette->_signalPaletteChanged();
    }
}

void VKPalette::_signalPaletteChanged() {
    emit paletteChanged();
}
