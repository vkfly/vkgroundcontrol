...
Item {
    ...
    function paintObstacleOverlay(ctx) {
        ...
        var grad = ctx.createRadialGradient(
                    centerX, centerY,
                    maxGradPixels - segmentHeightPixels * levelNum * 2,
                    centerX, centerY, maxGradPixels)
        ...
        for (var s = 0; s < segNum; ++s) {
            ...
            for (var ii = 0; ii < levelNum; ++ii) {
                ...
                if (rangeMin > rangeInLevel || isLast)
                    range = rangeMin
                const rangeToShow = showText
                                  && range < obstacleDistance._maxRadiusMeters ? range : 0
                drawSegment(ctx, rangeToShow, centerX, centerY, from, to,
                            radFrom, radTo, grad)
                if (range == rangeMin)
                    break
            }
