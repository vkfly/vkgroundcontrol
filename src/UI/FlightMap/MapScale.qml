...
Item {
    id: scale
    width: buttonsOnLeft
           || !_zoomButtonsVisible ? rightEnd.x + rightEnd.width : zoomDownButton.x
                                     + zoomDownButton.width
    ...
    property var _scaleLengthsFeet: [10, 25, 50, 100, 250, 500, 1000, 2000, 3000, 4000, 5280, 5280
        * 2, 5280 * 5, 5280 * 10, 5280 * 25, 5280 * 50, 5280 * 100, 5280
        * 250, 5280 * 500, 5280 * 1000]
    property bool _zoomButtonsVisible: zoomButtonsVisible
    ...
    function calculateScale() {
        if (mapControl) {
            ...
            var rightCoord = mapControl.toCoordinate(
                        Qt.point(scaleLinePixelLength, scale.y),
                        false /* clipToViewPort */
                        )
            var scaleLineMeters = Math.round(leftCoord.distanceTo(rightCoord))
            // if (VKGroundControl.settingsManager.unitsSettings.horizontalDistanceUnits.value === UnitsSettings.HorizontalDistanceUnitsFeet) {
    ...
    Rectangle {
        ...
        anchors.leftMargin: buttonsOnLeft
                            && (_zoomButtonsVisible
                                || terrainButtonVisible) ? ScreenTools.defaultFontPixelWidth / 2 : 0
        anchors.left: buttonsOnLeft ? (_zoomButtonsVisible ? zoomDownButton.right : (terrainButtonVisible ? terrainButton.right : parent.left)) : parent.left
        width: 2
    ...
    IconTextButton {
        ...
        iconSource: "/qmlimages/icon/terrain.svg"
        width: height
        opacity: 0.75
