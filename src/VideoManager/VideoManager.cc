...
void VideoManager::init(QQuickWindow *window) {
    ...
    for (const QString &streamName : videoStreamList) {
        VideoReceiver *receiver = createVideoReceiver(this);
        if (!receiver) {
            continue;
        }
        receiver->setName(streamName);
        void *sink = createVideoSink(receiver->widget(), receiver);
        if (!sink) {
...
VideoReceiver *VideoManager::createVideoReceiver(QObject *parent) {
#ifdef QGC_GST_STREAMING
...
void *VideoManager::createVideoSink(QQuickItem *widget, QObject *parent) {
#ifdef QGC_GST_STREAMING
...
void VideoManager::releaseVideoSink(void *sink) {
#ifdef QGC_GST_STREAMING
...
void VideoManager::cleanup() {
    for (VideoReceiver *receiver : std::as_const(_videoReceivers)) {
        releaseVideoSink(receiver->sink());
    }
}
