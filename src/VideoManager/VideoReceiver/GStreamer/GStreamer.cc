    ...
    void _setCodecPriorities(GStreamer::VideoDecoderOptions option) {
        ...
        switch (option) {
            ...
            case GStreamer::ForceVideoDecoderSoftware:
                for (const char *name : {
                            "avdec_h264", "avdec_h265", "avdec_mjpeg", "avdec_mpeg2video", "avdec_mpeg4", "avdec_vp8", "avdec_vp9",
            ...
            case GStreamer::ForceVideoDecoderNVIDIA:
                for (const char *name : {
                            "nvav1dec", "nvh264dec", "nvh265dec", "nvjpegdec", "nvmpeg2videodec", "nvmpeg4videodec", "nvmpegvideodec", "nvvp8dec", "nvvp9dec"
                        }) {
                    changeRank(name, PrioritizedRank);
                }
                break;
            ...
            case GStreamer::ForceVideoDecoderIntel:
                for (const char *name : {
                            "qsvh264dec", "qsvh265dec", "qsvjpegdec", "qsvvp9dec", "msdkav1dec", "msdkh264dec", "msdkh265dec", "msdkmjpegdec", "msdkmpeg2dec", "msdkvc1dec", "msdkvp8dec", "msdkvp9dec"
                        }) {
                    changeRank(name, PrioritizedRank);
                }
                break;
...
namespace GStreamer {
    ...
    void list_androidmedia_elements() {
        GstRegistry *registry = gst_registry_get();
        GList *features = gst_registry_get_feature_list_by_plugin(registry, "androidmedia");
        qCInfo(GStreamerLog) << "Elements in androidmedia plugin:";
        for (GList *l = features; l != NULL; l = l->next) {
            GstPluginFeature *feature = GST_PLUGIN_FEATURE(l->data);
            if (GST_IS_ELEMENT_FACTORY(feature)) {
                qCInfo(GStreamerLog) << "  " << gst_plugin_feature_get_name(feature);
            }
        }
        gst_plugin_feature_list_free(features);
        // 方法2：检查特定元素是否存在
        GstElementFactory *factory = gst_element_factory_find("amcviddec");
        if (factory) {
            qCInfo(GStreamerLog) << "amcviddec element exists";
            // 打印元素描述信息
            const gchar *desc = gst_element_factory_get_metadata(factory, GST_ELEMENT_METADATA_DESCRIPTION);
            qCInfo(GStreamerLog) << "Description: " << desc;
            // 打印支持的类型
            const GList *templates = gst_element_factory_get_static_pad_templates(factory);
            for (const GList *t = templates; t; t = t->next) {
                GstStaticPadTemplate *templ = (GstStaticPadTemplate *)t->data;
                GstCaps *caps = gst_static_pad_template_get_caps(templ);
                gchar *caps_str = gst_caps_to_string(caps);
                qCInfo(GStreamerLog) << "  Template: " << templ->name_template << " - Caps: " << caps_str;
                g_free(caps_str);
                gst_caps_unref(caps);
            }
            gst_object_unref(factory);
        } else {
            qCInfo(GStreamerLog) << "amcviddec element not found";
        }
    }
} // namespace GStreamer
