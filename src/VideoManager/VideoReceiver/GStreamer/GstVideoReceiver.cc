...
GstElement *GstVideoReceiver::_makeDecoder(GstCaps *caps, GstElement *videoSink) {
    Q_UNUSED(videoSink);
    GstElement *decodebin3 = gst_element_factory_make("decodebin3", nullptr);
    if (!decodebin3) {
        qCCritical(GstVideoReceiverLog) << "gst_element_factory_make('decodebin3') failed";
    }
#ifdef Q_OS_ANDROID
    // 创建一个 bin 来包含解析器和解码器
    GstElement *bin = gst_bin_new("decoderbin");
    if (!bin) {
        qCCritical(GstVideoReceiverLog) << "gst_bin_new('decoderbin') failed";
        return decodebin3;
    }
    // 检查是否可以使用 Android 硬件解码
    if (caps) {
        GstStructure *structure = gst_caps_get_structure(caps, 0);
        const gchar *mime = structure ? gst_structure_get_name(structure) : nullptr;
        // 确定媒体类型和对应的解析器
        const gchar *parserName = nullptr;
        const gchar *deCoderName = nullptr;
        if (mime) {
            if (g_strcmp0(mime, "video/x-h265") == 0) {
                parserName = "h265parse";
                deCoderName =  "amcviddec-omxgooglehevcdecoder";
                qCInfo(GstVideoReceiverLog) << "Setting up Android HEVC decoder";
            } else if (g_strcmp0(mime, "video/x-h264") == 0) {
                parserName = "h264parse";
                deCoderName =  "amcviddec-omxgooglehvcdecoder";
                qCInfo(GstVideoReceiverLog) << "Setting up Android H.264 decoder";
            }
        }
        // 如果是支持的媒体类型，尝试创建硬件解码路径
        if (parserName && deCoderName) {
            // 创建解析器
            GstElement *parser = gst_element_factory_make(parserName, nullptr);
            GstElement *decoder = parser ? gst_element_factory_make(deCoderName, nullptr) : nullptr;
            // 如果成功创建了解析器和解码器，设置连接
            if (parser && decoder) {
                // 添加到bin并连接
                ...
                // 如果链接或pad创建失败，移除元素并继续
                gst_bin_remove_many(GST_BIN(bin), parser, decoder, nullptr);
            } else {
                // 清理已创建的元素
                if (parser) {
