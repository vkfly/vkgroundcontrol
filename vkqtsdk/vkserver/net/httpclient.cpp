...
void HttpClientPrivate::executeQuery(HttpClientPrivate *d, HttpClientRequestMethod method) {
    ...
    QObject::connect(reply, &QNetworkReply::finished, [ = ] {
        QString message = HttpClientPrivate::readReply(reply, cache.charset.toUtf8());
        HttpClientPrivate::handleFinish(cache, reply, message, message);
        d->json = "";
        d->params = QUrlQuery();
    });
}
...
QNetworkRequest HttpClientPrivate::createRequest(HttpClientPrivate *d, HttpClientRequestMethod method) {
    ...
    QString targetUrl = d->url + d->path;
    // [1] 如果是 GET 请求，并且参数不为空，则编码请求的参数，放到 URL 后面
    if (get && !d->params.isEmpty()) {
        targetUrl += "?" + d->params.toString(QUrl::FullyEncoded);
    }
    // [2] 调试时输出网址和参数
    if (d->debug) {
        qDebug().noquote() << "[网址]" << targetUrl;
        if (withJson) {
...
HttpClient& HttpClient::path(const QString &path) {
    d->path = path;
    return *this;
}
...
void HttpClient::download(std::function<void (const QByteArray &)> readyRead) {
    HttpClientPrivate::download(d, readyRead);
}
// 上传文件
